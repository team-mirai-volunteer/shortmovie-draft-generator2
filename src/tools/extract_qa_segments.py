...
def save_qa_segments(qa_segments, output_path: str):
    ...
    output_data = {
        ...
        "extraction_info": {
            ...
            "average_duration": sum(seg.total_duration for seg in qa_segments)
            / len(qa_segments)
            if qa_segments
            else 0,
        },
...
def print_qa_summary(qa_segments):
    ...
    for segment in qa_segments:
        ...
        print(
            f"時間範囲: {segmenter.format_time_to_hms(segment.question.start_time)} - {segmenter.format_time_to_hms(segment.answer.end_time)}"
        )
        print(f"総時間: {segment.total_duration:.1f}秒")
        print(
            f"質問: {segment.question.text[:100]}{'...' if len(segment.question.text) > 100 else ''}"
        )
        print(
            f"回答: {segment.answer.text[:150]}{'...' if len(segment.answer.text) > 150 else ''}"
        )
        print("-" * 80)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="動画文字起こしから質問回答セグメントを抽出"
    )
    ...
    try:
        ...
        segmenter = QASegmenter()
        qa_segments = segmenter.extract_qa_segments(
            transcription_data, args.num_segments
        )
