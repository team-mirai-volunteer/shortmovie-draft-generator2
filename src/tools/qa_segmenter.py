...
class QASegmenter:
    ...
    def extract_qa_segments(self, transcription_data: Dict[str, Any], num_segments: int = 3) -> List[QASegment]:
        ...
        try:
            ...
            if len(question_markers) < num_segments:
                raise QASegmenterError(f"十分な質問セグメントが見つかりません。要求: {num_segments}, 発見: {len(question_markers)}")

            selected_questions = self._select_distributed_questions(question_markers, num_segments)
            
            qa_segments = []
            for i, question_info in enumerate(selected_questions):
                qa_segment = self._extract_single_qa(segments, question_info, i + 1, selected_questions)
                if qa_segment:
            ...
            raise QASegmenterError(f"Q&Aセグメント抽出中にエラーが発生しました: {str(e)}")

    def _find_question_markers(self, segments: List[Dict[str, Any]]) -> List[Tuple[int, Dict[str, Any], str]]:
        """質問マーカーを検出
    ...
    def _select_distributed_questions(self, question_markers: List[Tuple[int, Dict[str, Any], str]], 
                                    num_segments: int) -> List[Tuple[int, Dict[str, Any], str]]:
        """時間的に分散した質問を選択"""
        if len(question_markers) <= num_segments:
            return question_markers
    ...
    def _extract_single_qa(self, segments: List[Dict[str, Any]], 
                          question_info: Tuple[int, Dict[str, Any], str],
                          qa_id: int,
                          all_selected_questions: List[Tuple[int, Dict[str, Any], str]]) -> Optional[QASegment]:
        """単一のQ&Aセグメントを抽出"""
        question_idx, question_segment, speaker = question_info
        
        question_end_idx = self._find_question_end(segments, question_idx)
        
        answer_end_idx = self._find_answer_end(segments, question_end_idx, all_selected_questions)
        ...
        answer_text = self._build_text_range(segments, question_end_idx + 1, answer_end_idx)
        answer_start = segments[question_end_idx + 1]['start'] if question_end_idx + 1 < len(segments) else question_end
        answer_end = segments[answer_end_idx]['end'] if answer_end_idx < len(segments) else answer_start
    ...
    def _find_question_end(self, segments: List[Dict[str, Any]], question_start_idx: int) -> int:
        """質問の終了位置を特定"""
    ...
    def _find_answer_end(self, segments: List[Dict[str, Any]], answer_start_idx: int,
                        all_questions: List[Tuple[int, Dict[str, Any], str]]) -> int:
        """回答の終了位置を特定"""
    ...
    def _build_text_range(self, segments: List[Dict[str, Any]], start_idx: int, end_idx: int) -> str:
        """指定範囲のセグメントからテキストを構築"""
        texts = []
